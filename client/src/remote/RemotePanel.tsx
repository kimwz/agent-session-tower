...
export function RemoteButton({ token, projects, attention }: { token: string; projects: [string, string][]; attention?: boolean }) {
  const { t } = useI18n();
  const [open, setOpen] = useState(false);
  return <>
    <button className={`icon-button remote-button ${attention ? 'attention' : ''}`} aria-label={t('원격 컴퓨터')} title={t('원격 컴퓨터')} disabled={!token} onClick={() => setOpen(true)}><Network size={18} /></button>
...
export function RemotePanel({ token, projects, onClose }: { token: string; projects: [string, string][]; onClose: () => void }) {
  ...
  const refresh = useCallback(async () => {
    try { setOverview(await api<LinkOverview>('/api/link')); }
  ...
  const tabs = [['nodes', t('제어하는 컴퓨터'), Monitor], ['controllers', t('이 컴퓨터를 제어'), Radio], ['exclusions', t('공유 제외'), FolderX]] as const;
  return createPortal(<dialog ref={dialog} className="slack-dialog remote-dialog" aria-labelledby="remote-title" onCancel={event => { event.preventDefault(); onClose(); }}><div className="slack-panel">
    ...
    <div className="slack-body">
      ...
      {!overview ? <LoaderCircle className="spin" aria-label={t('불러오는 중')} />
        : tab === 'nodes' ? <Nodes token={token} overview={overview} busy={busy} run={run} />
...
function Nodes({ token, overview, busy, run }: { token: string; overview: LinkOverview; busy: boolean; run: Run }) {
  const { t } = useI18n();
  ...
  const known = useRef(new Set(overview.nodes.map(node => node.id)));
  const joined = invite ? overview.nodes.find(node => !known.current.has(node.id)) : undefined;
  ...
  return <section className="remote-section">
    ...
    {hub.enabled && hub.listening && <div className="remote-invite">
      {!invite || expired ? <button type="button" className="primary-button" disabled={busy} onClick={() => void run(async () => {
        known.current = new Set(overview.nodes.map(node => node.id));
        setInvite(await post<LinkInvite>(token, '/api/link/invite', {}));
      })}><Plus size={14} />{expired ? t('새 연결 명령 만들기') : t('컴퓨터 추가')}</button>
        : joined ? <p className="remote-joined" role="status"><Check size={14} />{t('{0}이(가) 연결되었습니다.', { 0: joined.label || joined.name })}</p>
...
function NodeRow({ token, node, busy, run }: { token: string; node: NodeSummary; busy: boolean; run: Run }) {
  ...
  return <li className="slack-rule-row remote-row">
    ...
    <div className="slack-rule-text">
      {editing ? <form className="remote-rename" onSubmit={save}><input autoFocus maxLength={80} value={label} placeholder={node.name} onChange={event => setLabel(event.target.value)} aria-label={t('표시 이름')} /><button className="secondary-button" disabled={busy}>{t('저장')}</button></form>
    ...
    <button type="button" className="icon-button" aria-label={t('{0} 연결 해제', { 0: node.label || node.name })} title={t('연결 해제')} disabled={busy}
      onClick={() => { if (window.confirm(t('{0}와의 연결을 해제할까요? 그 컴퓨터에서 진행 중인 작업은 계속됩니다. 다시 연결하려면 새 명령이 필요합니다.', { 0: node.label || node.name }))) void run(() => post(token, `/api/link/nodes/${node.id}/remove`, {})); }}><Trash2 size={15} /></button>
  </li>;
}
...
function Controllers({ token, overview, busy, run }: { token: string; overview: LinkOverview; busy: boolean; run: Run }) {
  ...
  const status = (item: ControllerSummary) => ({ connected: t('연결됨'), connecting: t('연결 중'), offline: t('연결 대기'), expired: t('연결 코드 만료'), refused: t('다른 컴퓨터가 응답함') }[item.status]);
  return <section className="remote-section">
    ...
    {overview.controllers.length ? <ol className="slack-rule-list">{overview.controllers.map(item => <li key={item.id} className="slack-rule-row remote-row">
      ...
      <div className="slack-rule-text"><strong>{item.name}</strong><small>{status(item)} · {t('지문')} {item.fingerprint}{item.error && item.status !== 'connected' ? ` · ${translateMessage(item.error)}` : ''}{item.lastConnectedAt ? ` · ${t('마지막 연결')} ${relativeTime(item.lastConnectedAt)}` : ''}</small></div>
      <button type="button" className="icon-button" aria-label={t('{0} 연결 해제', { 0: item.name })} title={t('연결 해제')} disabled={busy}
        onClick={() => { if (window.confirm(t('{0}이(가) 더 이상 이 컴퓨터를 제어하지 못하게 할까요? 진행 중인 작업은 계속됩니다.', { 0: item.name }))) void run(() => post(token, `/api/link/controllers/${item.id}/remove`, {})); }}><Trash2 size={15} /></button>
