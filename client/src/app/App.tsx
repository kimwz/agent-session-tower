...
function TowerApp() {
  ...
  // A clicked notification opens its conversation in the page that is already open.
  useEffect(() => {
    if (!('serviceWorker' in navigator)) return;
    const onMessage = (event: MessageEvent) => {
      if (event.data?.type !== 'tower:open' || typeof event.data.url !== 'string') return;
      const url = new URL(event.data.url, window.location.origin);
      if (url.origin === window.location.origin) selectSession(url.searchParams.get('session'));
    };
  ...
  const triggerTargets = useCallback((node: string) => ({
    ...
    sessions: mainSessions.filter(session => session.node === node).map(session => ({ ...session, id: localPart(session.id), cwd: localPart(session.cwd) })),
  }), [projects, mainSessions]);
