...
export function App() {
  ...
  useEffect(() => {
    const inCanvasContext = (element: Element | null) => {
      if (!element || element.closest(`${shortcutEditingControls}, .sidebar, .chat-panel, .help-popover, dialog, [role="dialog"]`)) return false;
  ...
  const visibleWorking = canvasSessions.filter(session => session.status === 'working').length;
  ...
  const filterControls = <>
    ...
    <label className="show-hidden-toggle" title={t("숨긴 폴더 포함 (Shift+A)")}><input type="checkbox" checked={showHidden} onChange={event => setShowHidden(event.target.checked)} aria-label={t("전체보기")} aria-keyshortcuts="Shift+A" /><span>{t("전체보기")}</span><kbd>⇧A</kbd></label>
    <div className="canvas-status"><span className={`status-pip ${visibleWorking ? 'working' : 'completed'}`} /><span>{canvasSessions.length.toLocaleString()}{t("개 세션")}{visibleWorking > 0 && t(" · {0}개 작업 중", { 0: visibleWorking })}</span>{snapshot?.scanning && <LoaderCircle size={11} className="spin" />}</div>
  </>;
  const canvasEmptyState = !canvasSessions.length && !visiblePins.length && <div className="graph-empty canvas-empty">
    <h3>{hasHiddenMatches ? t("폴더가 숨겨져 있습니다") : mainSessions.length ? t("표시할 세션이 없습니다") : t("새 세션을 시작해 보세요")}</h3>
    <p>{hasHiddenMatches ? t("전체보기를 켜면 현재 필터에 맞는 숨긴 폴더와 세션을 볼 수 있습니다.") : mainSessions.length ? t("검색어나 필터를 바꾸면 다른 세션을 볼 수 있습니다.") : t("Claude Code 또는 Codex를 선택해 이곳에서 작업을 시작할 수 있습니다.")}</p>
    ...
    <div className="workspace">
      ...
      <main ref={canvasRef} className="main-area" tabIndex={-1} onPointerDownCapture={event => {
        ...
        {!snapshot || (snapshot.scanning && sessions.length === 0 && !visiblePins.length) ? <>
          <div className="canvas-toolbar"><div className="canvas-filter-row" aria-label={t("캔버스 필터")}>{filterControls}</div></div>
