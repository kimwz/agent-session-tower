...
function SessionRow({ session, selected, unread, onSelect }: { session: Session; selected: boolean; unread: boolean; onSelect: (id: string) => void }) {
  ...
  return <button className={`session-row ${selected ? 'selected' : ''} ${unread ? 'unread' : ''}`} onClick={() => onSelect(session.id)} aria-pressed={selected}>
    ...
    <span className="session-row-main"><span className="session-row-heading"><span className="session-row-project folder-tail" title={session.cwd || session.project || '프로젝트 없음'}><bdi dir="ltr">{session.project || '프로젝트 없음'}</bdi></span><time dateTime={activityAt}>{relativeTime(activityAt)}</time></span><strong>{unread && <i className="unread-dot" title="새 활동" aria-label="새 활동" />}{sessionTitle(session)}</strong><span className="session-row-preview">{cleanPreview(session.lastMessage, 100) || providerLabels[session.provider]}</span><span className="session-row-meta"><span className={`row-status ${session.status}`}>{statusLabels[session.status]}</span>{session.isSubagent && <><GitBranch size={10} /><span>하위 에이전트</span></>}{selected && <ArrowUpRight size={12} />}</span></span>
  </button>;
}
...
export function App() {
  ...
  const refresh = useCallback(() => {
    setRefreshing(true);
    void api<Snapshot>('/api/snapshot').then(value => { setSnapshot(value); setLoadError(''); }).catch(error => setLoadError(error instanceof Error ? error.message : '서버에 연결하지 못했습니다.')).finally(() => setRefreshing(false));
  ...
  useEffect(() => {
    ...
    events.addEventListener('snapshot', event => {
      try { setSnapshot(JSON.parse((event as MessageEvent<string>).data) as Snapshot); setConnection('connected'); setLoadError(''); } catch { setLoadError('세션 업데이트를 읽지 못했습니다. 새로고침해 주세요.'); }
    });
  ...
  const updateGroup = useCallback(async (patch: ProjectGroupPatch): Promise<boolean> => {
    ...
    try {
      ...
      setGroupErrors(previous => ({ ...previous, [patch.cwd]: error instanceof Error ? error.message : '그룹을 저장하지 못했습니다. 다시 시도해 주세요.' }));
      return false;
    } finally {
  ...
  const projects = useMemo(() => projectGroupChoices(allMainSessions, groups), [allMainSessions, groups]);
  ...
  return <div className={`app ${selectedId ? 'has-chat' : ''}`}>
    <header className="app-header"><div className="brand"><button ref={sidebarToggleRef} className="icon-button sidebar-toggle" onClick={() => sidebarIsDrawer ? setShowSidebar(value => !value) : setSidebarCollapsed(value => !value)} aria-label={sidebarOpen ? '세션 목록 접기' : '세션 목록 열기'} title={sidebarOpen ? '세션 목록 접기' : '세션 목록 열기'} aria-controls="session-sidebar" aria-expanded={sidebarOpen}>{sidebarOpen ? <PanelLeftClose size={19} /> : <PanelLeftOpen size={19} />}</button><span className="brand-mark"><BrandMark /></span><h1>Agent Session Tower</h1><span className="brand-local">local</span></div><div className="header-center"><Monitor size={13} /><span>{snapshot?.hostname || '이 Mac'}</span><span className="header-divider" /><span className="localhost">{window.location.host}</span></div><div className="header-actions"><button className="new-session-button" onClick={() => openNewSession()} disabled={!token || connection !== 'connected'}><Plus size={15} /><span>새 세션</span></button><span className={`connection-state ${connection}`} role="status">{connection === 'offline' ? <WifiOff size={12} /> : <i />}{connection === 'connected' ? '실시간 연결' : connection === 'offline' ? '재연결 중' : '연결 중'}</span><button className={`icon-button ${showHelp ? 'active' : ''}`} title="사용 안내" aria-label="사용 안내" aria-expanded={showHelp} onClick={() => setShowHelp(!showHelp)}><CircleHelp size={18} /></button></div></header>
    {connection === 'offline' && <div className="connection-banner" role="alert"><WifiOff size={14} /><span>Monitor와의 연결이 끊겼습니다. 저장된 화면을 표시하며 자동으로 다시 연결합니다.</span><button onClick={refresh}>다시 확인</button></div>}
    {showHelp && <div className="help-popover"><div className="help-heading"><h2>내 컴퓨터의 에이전트를 한눈에</h2><button className="icon-button" onClick={() => setShowHelp(false)} aria-label="안내 닫기"><X size={15} /></button></div><p>Claude Code와 Codex의 로컬 세션 기록을 자동으로 읽습니다. 그래프의 에이전트를 선택하면 실제 대화를 보고 작업을 이어갈 수 있습니다.</p><div className="help-statuses"><span><i className="legend-dot working" /><b>작업 중</b>현재 실행 중인 작업</span><span><i className="legend-dot idle" /><b>대기 중</b>입력이나 다음 작업 대기</span><span><i className="legend-dot completed" /><b>완료</b>작업 종료가 기록된 세션</span></div><p>상태는 프로세스와 세션 기록을 함께 확인합니다. 대화 상단의 프로젝트 이름을 누르면 상태 판단 근거를 볼 수 있습니다.</p><div className="help-privacy"><ShieldCheck size={16} /><span>로컬에서 실행됩니다. 새 요청은 해당 CLI와 기존 로그인 계정을 사용합니다.</span></div><div className="help-shortcuts"><span><kbd>/</kbd> 세션 검색</span><span><kbd>Esc</kbd> 대화 닫기</span><span><kbd>⌘ Enter</kbd> 요청 보내기</span></div></div>}
    <div className="workspace">
      {sidebarIsDrawer && sidebarOpen && <button className="sidebar-scrim" aria-label="세션 목록 닫기" onClick={() => setShowSidebar(false)} />}
      <aside id="session-sidebar" className={`sidebar ${sidebarOpen ? 'open' : ''}`} hidden={!sidebarOpen} aria-label="세션 탐색"><div className="sidebar-heading"><h2>세션</h2><span>{mainSessions.length.toLocaleString()}</span><button className="icon-button refresh-button" onClick={refresh} title="세션 새로고침" aria-label="세션 새로고침" disabled={refreshing}><RefreshCw size={14} className={refreshing ? 'spin' : ''} /></button></div><div className="search-wrap"><Search size={15} /><input ref={searchRef} type="search" placeholder="세션, 프로젝트 검색" aria-label="세션 검색" value={query} onChange={event => setQuery(event.target.value)} /><kbd>/</kbd></div><div className="provider-filters" aria-label="에이전트 종류"><button className={provider === 'all' ? 'selected' : ''} onClick={() => setProvider('all')} aria-pressed={provider === 'all'}>전체</button><button className={provider === 'claude' ? 'selected claude' : ''} onClick={() => setProvider('claude')} aria-pressed={provider === 'claude'}><ProviderIcon provider="claude" size={13} />Claude</button><button className={provider === 'codex' ? 'selected codex' : ''} onClick={() => setProvider('codex')} aria-pressed={provider === 'codex'}><ProviderIcon provider="codex" size={13} />Codex</button></div><div className="sidebar-selects"><label><Folder size={13} /><select aria-label="프로젝트 필터" value={project} onChange={event => setProject(event.target.value)}><option value="all">모든 프로젝트</option>{projects.map(([path, name]) => <option key={path} value={path}>{name}</option>)}</select><ChevronDown size={11} /></label><label><select aria-label="세션 상태 필터" value={status} onChange={event => setStatus(event.target.value as StatusFilter)}><option value="all">모든 상태</option><option value="working">작업 중</option><option value="idle">대기 중</option><option value="completed">완료</option><option value="error">중단·오류</option></select><ChevronDown size={11} /></label></div><button className={`closed-sessions-toggle ${showClosed ? 'selected' : ''}`} onClick={() => { setShowClosed(value => !value); setListLimit(80); }} aria-pressed={showClosed}><Archive size={12} /><span>{showClosed ? '열린 세션 보기' : '종료한 세션'}</span><b>{closedSessions.length}</b></button><div className="session-list-label"><span>{showClosed ? '종료한 세션' : hasFilters ? '검색 결과' : '최근 활동'}</span><span>{listedSessions.length.toLocaleString()}개{hasFilters && <button onClick={() => { setQuery(''); setProvider('all'); setProject('all'); setStatus('all'); }} aria-label="검색 및 필터 초기화"><X size={12} /></button>}</span></div><div className="session-list">
        {!snapshot ? <div className="sidebar-loading"><LoaderCircle className="spin" size={17} /><span>세션을 찾고 있습니다</span></div> : listedSessions.length ? <>{listedSessions.slice(0, listLimit).map(session => <SessionRow key={session.id} session={session} selected={selectedMainId === session.id} unread={unreadIds.has(session.id)} onSelect={onSelect} />)}{listedSessions.length > listLimit && <button className="load-more-sessions" onClick={() => setListLimit(value => value + 80)}>세션 더 보기 <ChevronDown size={13} /><span>{Math.min(listLimit, listedSessions.length)} / {listedSessions.length.toLocaleString()}</span></button>}</> : <div className="sidebar-empty"><Search size={22} /><span>{showClosed ? '종료한 세션이 없습니다' : sessions.length ? '조건에 맞는 세션이 없습니다' : '아직 발견된 세션이 없습니다'}</span>{sessions.length > 0 && <button onClick={clearFilters}>전체 기록 보기</button>}</div>}
      </div><div className="providers-health"><div className="providers-health-label"><Terminal size={12} /><span>로컬 에이전트</span>{snapshot?.scanning && <LoaderCircle size={11} className="spin" />}</div>{(['claude', 'codex'] as const).map(name => { const health = snapshot?.providers.find(item => item.provider === name); return <div className="provider-health" key={name} title={health?.error || health?.executable || `${providerLabels[name]} 상태 확인 중`}><ProviderIcon provider={name} size={13} /><span>{providerLabels[name]}</span><span className={health?.available ? 'available' : 'unavailable'}>{health ? health.available ? <><Check size={10} />사용 가능</> : 'CLI 없음' : '확인 중'}</span></div>; })}<p><ShieldCheck size={10} />이 Mac의 에이전트와 연결됩니다</p></div></aside>
      <main className="main-area">
        <div className="canvas-toolbar" aria-label="캔버스 필터">
          <div className="stat-filters">
            <button className={status === 'all' ? 'selected' : ''} onClick={() => setStatus('all')} aria-pressed={status === 'all'}><Network size={12} /><span>전체</span><b>{mainSessions.length.toLocaleString()}</b></button>
            <button className={`working ${status === 'working' ? 'selected' : ''}`} onClick={() => setStatus(status === 'working' ? 'all' : 'working')} aria-pressed={status === 'working'}><Activity size={12} /><span>작업 중</span><b>{working}</b></button>
            <button className={status === 'completed' ? 'selected' : ''} onClick={() => setStatus(status === 'completed' ? 'all' : 'completed')} aria-pressed={status === 'completed'}><CheckCheck size={13} /><span>완료</span><b>{completed.toLocaleString()}</b></button>
          </div>
          <label className="period-select"><select aria-label="세션 조회 기간" value={period} onChange={event => setPeriod(event.target.value)}><option value="1">최근 24시간</option><option value="7">최근 7일</option><option value="30">최근 30일</option><option value="all">전체 기록</option></select><ChevronDown size={12} /></label>
        </div>
        {loadError && <div className="main-error canvas-error" role="alert"><TriangleAlert size={15} /><span>{loadError}</span><button onClick={refresh}>다시 시도</button></div>}
        {!snapshot || (snapshot.scanning && sessions.length === 0 && !visiblePins.length) ? <div className="graph-loading"><div className="loading-constellation"><span /><span /><span /><Monitor size={25} /></div><h3>이 Mac의 에이전트를 찾고 있습니다</h3><p>Claude Code와 Codex의 실제 세션 기록을 연결합니다.</p></div> : <>
          <Graph sessions={filtered} allSessions={mainSessions} sessionsReady={!snapshot.scanning} unreadIds={unreadIds} selectedId={selectedMainId} hostname={snapshot.hostname} onSelect={onSelect} filterKey={filterKey} groups={groups} visiblePins={visiblePins} groupSaving={groupSaving} groupErrors={groupErrors} groupActionsDisabled={!token || connection !== 'connected'} onGroupUpdate={updateGroup} onGroupCreate={openNewSession} />
          {!filtered.length && !visiblePins.length && <div className="graph-empty canvas-empty"><h3>{mainSessions.length ? '표시할 세션이 없습니다' : '새 세션을 시작해 보세요'}</h3><p>{mainSessions.length ? '검색어나 필터를 바꾸면 다른 세션을 볼 수 있습니다.' : 'Claude Code 또는 Codex를 선택해 이곳에서 작업을 시작할 수 있습니다.'}</p>{mainSessions.length ? <button className="secondary-button" onClick={clearFilters}><RefreshCw size={13} />전체 기록 보기</button> : <button className="secondary-button" onClick={() => openNewSession()} disabled={!token || connection !== 'connected'}><Plus size={13} />새 세션</button>}</div>}
        </>}
        <div className="canvas-status"><span className={`status-pip ${visibleWorking ? 'working' : 'completed'}`} /><span>{filtered.length.toLocaleString()}개 세션{visibleWorking > 0 && ` · ${visibleWorking}개 작업 중`}</span>{snapshot?.scanning && <LoaderCircle size={11} className="spin" />}</div>
      </main>
      {selectedId && <Suspense fallback={<aside className="chat-panel"><div className="chat-loading"><LoaderCircle className="spin" size={20} /><span>대화를 여는 중</span></div></aside>}><ChatPanel key={selectedId} sessionId={selectedId} session={selectedSession} allSessions={sessions} provider={snapshot?.providers.find(item => item.provider === (selectedSession?.provider || (selectedId.startsWith('claude') ? 'claude' : 'codex')))} runs={currentRuns} token={token} connected={connection === 'connected'} onClose={closeChat} onNavigate={onSelect} onSnapshotRefresh={refresh} onSessionUpdate={onSessionUpdate} onSessionClose={changeSessionClosed} sessionClosed={!!selectedMainSession?.closed} changingClosed={changingClosed} readRevision={showNewSession || (sidebarIsDrawer && sidebarOpen) ? '' : revisions.get(selectedId)} onRead={onRead} /></Suspense>}
    </div>
