...
export const statusLabels: Record<SessionStatus, string> = {
  working: '작업 중', idle: '대기 중', completed: '완료', error: '중단·오류',
};
...
export function absoluteTime(value: string) {
  ...
  return Number.isNaN(date.getTime()) ? '' : date.toLocaleString('ko-KR', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit', hour12: false });
}
export function sessionTitle(session: Session) {
  ...
  return (agentName && !/^[a-f0-9]{12,}$/i.test(agentName) ? agentName : session.title) || '제목 없는 세션';
}
