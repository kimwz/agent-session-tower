...
import { addDraftFiles, formatAttachmentSize, prepareDraftAttachments, savedAttachmentDraft } from './chat-attachments';
import { MAX_ATTACHMENTS, MAX_TOTAL_ATTACHMENT_BYTES } from '../../shared/attachments';
import { finishComposerSend, getComposerState, markComposerSending, setComposerDraft, setComposerError, startComposerSend, subscribeComposer, type ChatDraft } from './chat-drafts';
...
function CodeBlock({ children, ...props }: ComponentPropsWithoutRef<'pre'>) {
  ...
  return <div className="code-block"><button aria-label="코드 복사" onClick={() => { void copyText(ref.current?.textContent || '').then(success => { setCopied(success); window.setTimeout(() => setCopied(false), 1500); }); }}>{copied ? <Check size={13} /> : <Copy size={13} />}{copied ? '복사됨' : '복사'}</button><pre {...props} ref={ref}>{children}</pre></div>;
}
const Markdown = memo(function Markdown({ children }: { children: string }) {
  return <div className="markdown"><ReactMarkdown remarkPlugins={[remarkGfm]} components={{ pre: CodeBlock, a: ({ children, ...props }) => <a {...props} target="_blank" rel="noreferrer noopener">{children}</a>, img: ({ alt }) => <span className="attachment-label">[이미지: {alt || '첨부 파일'}]</span> }}>{children}</ReactMarkdown></div>;
});

export const Message = memo(function Message({ message, runMatch }: { message: ChatMessage; runMatch?: ChatRunMatch }) {
  const [expanded, setExpanded] = useState(false);
  if (message.role === 'tool' || message.role === 'system') return <details className={`tool-message ${message.isError ? 'tool-error' : ''}`} open={expanded} onToggle={event => setExpanded(event.currentTarget.open)}><summary><Terminal size={13} /><strong>{message.toolName || (message.role === 'system' ? '세션 정보' : '도구 실행')}</strong><span>{cleanPreview(message.text, 110)}</span><ChevronDown size={13} /></summary>{expanded && <div><Markdown>{message.text}</Markdown></div>}</details>;
  ...
  return <article className={`chat-message ${message.role} ${message.isError ? 'message-error' : ''}`}>
    <div className="message-meta"><span>{message.role === 'user' ? '나' : '에이전트'}</span><time dateTime={message.timestamp}>{absoluteTime(message.timestamp)}</time></div>
    ...
    {long && <button className="text-button message-expand" onClick={() => setExpanded(!expanded)}>{expanded ? '간략히 보기' : '메시지 전체 보기'}<ChevronDown size={12} className={expanded ? 'rotate' : ''} /></button>}
  </article>;
});
...
export const ToolMessageGroup = memo(function ToolMessageGroup({ group }: { group: ToolGroup }) {
  ...
  return <details className={`tool-group ${group.errorCount ? 'tool-group-error' : ''}`} open={expanded} onToggle={event => {
    ...
    <summary ref={summary}><strong>{group.workCount ? `${group.workCount}개의 작업` : '세션 정보'}</strong>{group.errorCount > 0 && <span className="tool-group-errors"><TriangleAlert size={10} aria-hidden="true" />오류 {group.errorCount}건</span>}<ChevronDown size={11} aria-hidden="true" /></summary>
    {expanded && <div className="tool-group-content">{group.messages.map(message => <Message key={message.id} message={message} />)}<button className="tool-group-collapse" onClick={() => { setExpanded(false); summary.current?.focus(); }}>{group.workCount ? '작업 접기' : '세션 정보 접기'}<ChevronDown size={11} aria-hidden="true" /></button></div>}
  </details>;
});
...
export const RunControl = memo(function RunControl({ run, onCancel, onRetry, cancelling, onDismiss, dismissing = false, disabled = false, retryDisabled = false, showPrompt = true }: { run: Run; onCancel: (id: string) => void; onRetry: (run: Run) => void; cancelling: boolean; onDismiss?: (id: string) => void; dismissing?: boolean; disabled?: boolean; retryDisabled?: boolean; showPrompt?: boolean }) {
  ...
  const preview = run.prompt || (run.attachments?.length ? run.attachments.map(file => file.name).join(', ') : '보낸 요청');
  return <div data-run-id={run.id} className={`run-control ${run.status}`}>
    <div className="run-control-line">
      ...
      <strong>{run.status === 'queued' ? '전송 대기 중' : run.status === 'running' ? '작업 중' : '요청 실패'}</strong>
      ...
      <div className="run-control-actions">
        {active && <button type="button" disabled={disabled || cancelling} onClick={() => onCancel(run.id)}><Square size={10} aria-hidden="true" />{cancelling ? '취소 중…' : run.status === 'queued' ? '대기 취소' : '중지'}</button>}
        {run.status === 'error' && <><button type="button" aria-label="요청 다시 작성" disabled={disabled || retryDisabled} onClick={() => onRetry(run)}><RefreshCw size={11} aria-hidden="true" />다시 작성</button>{onDismiss && <button type="button" aria-label="실패 내역 지우기" disabled={disabled || dismissing} onClick={() => onDismiss(run.id)}>{dismissing ? <LoaderCircle className="spin" size={11} aria-hidden="true" /> : <Trash2 size={11} aria-hidden="true" />}지우기</button>}</>}
      </div>
    </div>
    {run.status === 'error' && run.error && <p className="run-control-error">{run.error}</p>}
  </div>;
});
...
export function ChatPanel({ sessionId, session, allSessions, provider, runs, token, connected, onClose, onNavigate, onSnapshotRefresh, onSessionUpdate, onSessionClose, sessionClosed = false, changingClosed = false, readRevision = '', onRead }: { sessionId: string; session?: Session; allSessions: Session[]; provider?: ProviderHealth; runs: Run[]; token: string; connected: boolean; onClose: () => void; onNavigate: (id: string) => void; onSnapshotRefresh: () => void; onSessionUpdate: (session: Session) => void; onSessionClose?: () => void; sessionClosed?: boolean; changingClosed?: boolean; readRevision?: string; onRead?: (id: string, revision: string) => void }) {
  ...
  useEffect(() => {
    ...
    const read = () => {
      void api<SessionDetail>(`/api/sessions/${encodeURIComponent(sessionId)}?limit=100`, { signal: controller.signal }).then(next => {
        ...
        if (controller.signal.aborted) return;
        setLoadError(error instanceof Error ? error.message : '대화 기록을 읽지 못했습니다.'); setLoading(false);
      });
    };
  ...
  async function loadOlder() {
    ...
    } catch (error) { setLoadError(error instanceof Error ? error.message : '이전 대화를 불러오지 못했습니다.'); }
  ...
  function addFiles(files: File[]) {
    ...
    try {
      ...
      setSendError('');
    } catch (error) { setSendError(error instanceof Error ? error.message : '파일을 첨부하지 못했습니다.'); }
  }
  ...
  async function sendPrompt() {
    ...
    try {
      ...
      await api<{ run: Run }>(`/api/sessions/${encodeURIComponent(id)}/messages`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-Agent-Monitor-Token': token }, body: JSON.stringify({ prompt: message, ...prepared }) });
      finishComposerSend(id, submitted);
      if (mounted.current && sessionRef.current === id) { followRef.current = true; setFollowing(true); }
      onSnapshotRefresh();
    } catch (error) { finishComposerSend(id, submitted, error instanceof Error ? error.message : '요청을 보내지 못했습니다.'); }
  }
  ...
  const cancelRun = useCallback(async (id: string) => {
    ...
    catch (error) { setSendError(error instanceof Error ? error.message : '작업을 중지하지 못했습니다.'); }
  ...
  const dismissRun = useCallback(async (id: string) => {
    ...
    try {
      ...
      onSnapshotRefresh();
    } catch (error) { if (sessionRef.current === requestedSession) setSendError(error instanceof Error ? error.message : '실패 내역을 지우지 못했습니다.'); }
    finally { setDismissing(''); }
  }, [connected, onSnapshotRefresh, setSendError, token]);
  const retryPrompt = useCallback((run: Run) => {
    if (getComposerState(sessionId).stage) return;
    const next = { prompt: run.prompt, attachments: (run.attachments || []).map(savedAttachmentDraft) };
    setComposerDraft(sessionId, next); setComposerError(sessionId, ''); composer.current?.focus();
  }, [sessionId]);

  let composerHint: ReactNode = <><kbd>⌘</kbd><kbd>Enter</kbd><span>전송</span></>;
  if (!connected) composerHint = '서버에 다시 연결되면 요청을 보낼 수 있습니다.';
  else if (current?.creationPending) composerHint = '새 세션을 시작하고 있습니다.';
  else if (current && !current.resumable) composerHint = '이 세션은 대화 기록만 확인할 수 있습니다.';
  else if (provider && !provider.available) composerHint = `${current ? providerLabels[current.provider] : '에이전트'} CLI를 설치하면 이어서 작업할 수 있습니다.`;
  else if (!token) composerHint = '연결을 준비하는 중…';
  const disabled = !connected || !current?.resumable || !provider?.available || !token;
  const sendingLabel = stage === 'preparing' ? '파일 준비 중' : '보내는 중';

  return <aside className="chat-panel" aria-label="세션 대화" onKeyDown={event => { if (event.key === 'Escape' && !event.nativeEvent.isComposing) onClose(); }}>
    <header className="chat-header"><div className="chat-heading-line"><button className="icon-button mobile-back" onClick={onClose} aria-label="그래프로 돌아가기"><ChevronLeft size={20} /></button>{current && <span className={`provider-square ${current.provider} ${current.status}`}><ProviderIcon provider={current.provider} /></span>}<span>{current ? providerLabels[current.provider] : '세션 대화'}</span>{current && <span className={`status-badge ${current.status}`} title={current.statusReason}><i />{statusLabels[current.status]}</span>}{onSessionClose && <button className="icon-button session-close-button" aria-label={sessionClosed ? '세션 다시 열기' : '세션 종료'} title={sessionClosed ? '그래프에 다시 표시' : '그래프에서 숨기기 · 실행 중인 작업은 계속됩니다'} disabled={changingClosed || !connected || !token} onClick={() => { setSendError(''); void Promise.resolve(onSessionClose()).catch(error => setSendError(error instanceof Error ? error.message : '세션 상태를 저장하지 못했습니다.')); }}>{changingClosed ? <LoaderCircle className="spin" size={15} /> : sessionClosed ? <ArchiveRestore size={16} /> : <Archive size={16} />}</button>}<button className="icon-button close-chat" onClick={onClose} aria-label="대화 닫기" title="닫기 (Esc)"><X size={18} /></button></div>{current ? <SessionTitleEditor key={current.id} session={current} token={token} connected={connected} onSaved={updated => { onSessionUpdate(updated); setDetail(previous => previous ? { ...previous, session: { ...previous.session, customTitle: updated.customTitle } } : previous); onSnapshotRefresh(); }} /> : <h2>대화 불러오는 중</h2>}<div className="chat-context-row"><button className="chat-project" onClick={() => setShowMetadata(!showMetadata)} aria-expanded={showMetadata}><Folder size={12} /><span className="folder-tail" title={current?.cwd || current?.project || '세션 정보'}><bdi dir="ltr">{current?.project || '세션 정보'}</bdi></span>{current?.isSubagent && <><GitBranch size={12} /><span>하위 에이전트</span></>}<ChevronDown size={12} className={showMetadata ? 'rotate' : ''} /></button><SessionFamilyNav sessions={allSessions} selectedId={sessionId} onNavigate={onNavigate} /></div>{showMetadata && current && <dl className="session-metadata"><div><dt>작업 폴더</dt><dd>{current.cwd || '정보 없음'}</dd></div>{current.model && <div><dt>모델</dt><dd>{current.model}</dd></div>}<div><dt>세션 ID</dt><dd>{current.nativeId}<button className="icon-button" title="세션 ID 복사" aria-label="세션 ID 복사" onClick={() => { void copyText(current.nativeId).then(success => { setCopied(success); window.setTimeout(() => setCopied(false), 1500); }); }}>{copied ? <Check size={12} /> : <Copy size={12} />}</button></dd></div><div><dt>상태 판단</dt><dd>{current.statusReason}</dd></div><div><dt>시작</dt><dd>{absoluteTime(current.createdAt)}</dd></div></dl>}</header>
    <div className="chat-scroll" ref={scroller} onScroll={() => { const el = scroller.current; if (!el) return; const near = el.scrollHeight - el.scrollTop - el.clientHeight < 100; followRef.current = near; setFollowing(near); }}>
      {loading && !detail ? <div className="chat-loading"><LoaderCircle className="spin" size={22} /><span>대화 기록을 불러오는 중</span></div> : <>
        {loadError && <div className="chat-load-error"><TriangleAlert size={18} /><p>{loadError}</p><button className="secondary-button" onClick={() => setRetry(value => value + 1)}><RefreshCw size={13} />다시 불러오기</button></div>}
        {detail?.hasMore && <button className="load-older" onClick={() => { void loadOlder(); }} disabled={loadingOlder}>{loadingOlder ? <LoaderCircle className="spin" size={13} /> : <ArrowUp size={13} />}{loadingOlder ? '불러오는 중…' : '이전 대화 불러오기'}</button>}
        {detail?.resetToLatest && <p className="muted" role="status">대화가 갱신되어 최근 내용부터 표시합니다.{detail.hasMore && ' 앞선 내용은 ‘이전 대화 불러오기’에서 확인할 수 있습니다.'}</p>}
        {detail && !detail.hasMore && detail.messages.length > 0 && <div className="conversation-start"><span>세션 시작</span><time>{absoluteTime(detail.session.createdAt)}</time></div>}
        {detail && <ChatTranscript key={sessionId} messages={detail.messages} runMatches={runProjection.matches} />}
        {detail && detail.messages.length === 0 && !loadError && <div className="empty-chat"><MessageSquare size={27} /><h3>대화가 시작될 자리</h3><p>이 세션에 첫 요청을 보내거나<br />에이전트의 활동을 기다리세요.</p></div>}
      </>}
    </div>
    {!following && <button className="jump-latest" onClick={scrollToBottom}><ArrowDown size={13} />최신 대화</button>}
    <div className="composer-section">
      {current?.resumable === false && current.parentId && <button className="parent-session-link" onClick={() => onNavigate(current.parentId!)}><GitBranch size={14} /><span>부모 세션에서 이어가기</span><ArrowUp size={13} /></button>}
      {sendError && <div className="inline-error" role="alert"><TriangleAlert size={15} /><span>{sendError}</span><button aria-label="오류 메시지 닫기" onClick={() => setSendError('')}><X size={13} /></button></div>}
      {controlRuns.length > 0 && <div className="run-controls" aria-label="진행 중이거나 실패한 요청">{controlRuns.map(run => <RunControl key={run.id} run={run} onCancel={cancelRun} onRetry={retryPrompt} onDismiss={dismissRun} cancelling={cancelling === run.id} dismissing={dismissing === run.id} disabled={!connected || !token || !!cancelling || !!dismissing} retryDisabled={disabled || sending} showPrompt={!runProjection.matchedRunIds.has(run.id)} />)}</div>}
      <form className={`composer ${disabled ? 'disabled' : ''} ${dragging ? 'composer-dragging' : ''}`} onSubmit={event => { event.preventDefault(); void sendPrompt(); }}
        ...
        <input ref={fileInput} className="attachment-file-input" type="file" multiple aria-label="첨부할 파일 선택" disabled={disabled || sending} onChange={event => { addFiles(Array.from(event.currentTarget.files || [])); event.currentTarget.value = ''; }} />
        ...
        {dragging && <div className="attachment-drop-hint" aria-live="polite"><Paperclip size={14} />파일을 놓아 첨부하기</div>}
        <textarea ref={composer} aria-label="에이전트에게 보낼 요청" placeholder={current?.resumable === false ? '이 세션은 읽기 전용입니다' : '이 세션에서 이어서 작업하기…'} value={prompt} disabled={disabled || sending} rows={3} maxLength={32000}
        ...
        <div className="composer-bottom">
          <button className="attach-button" type="button" aria-label="파일 첨부" title={`파일 첨부 · 최대 ${MAX_ATTACHMENTS}개, 합계 ${formatAttachmentSize(MAX_TOTAL_ATTACHMENT_BYTES)} · 이미지 붙여넣기 가능`} disabled={disabled || sending} onClick={() => fileInput.current?.click()}><Paperclip size={16} aria-hidden="true" /></button>
          <span className="composer-hint">{sending ? <span role="status">{sendingLabel}…</span> : prompt.length > 24000 ? `${prompt.length.toLocaleString()} / 32,000자` : composerHint}</span>
          <button className="send-button" type="submit" disabled={disabled || (!prompt.trim() && !attachments.length) || sending} aria-label={sending ? '요청 보내는 중' : '요청 보내기'}>{sending ? <LoaderCircle className="spin" size={14} /> : <Send size={14} />}<span>{sending ? sendingLabel : '보내기'}</span></button>
        </div>
      </form>
      <p className="composer-note"><Terminal size={11} />이 Mac의 {current ? providerLabels[current.provider] : '에이전트'}에서 기존 대화를 이어갑니다.</p>
    </div>
  </aside>;
}
