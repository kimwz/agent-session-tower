...
export function SessionTail({ sessionId, selection, onNavigate }: { sessionId: string; selection: string; onNavigate: (id: string) => void }) {
  ...
  return <section><h3>{t('에이전트 응답')}</h3>
    ...
    {detail ? <><p className="slack-monitor-muted">{t('실행 세션의 최근 대화입니다. 전체 기록은 실행 세션에서 확인하세요.')}</p><ChatTranscript messages={detail.messages} provider={detail.session.provider} /></> : !error && <p>{t('대화를 여는 중')}</p>}
  </section>;
}
