...
const AttachmentTile = memo(function AttachmentTile({ attachment, onRemove, disabled }: { attachment: DraftAttachment; onRemove?: (key: string) => void; disabled?: boolean }) {
  ...
  return <li className={`attachment-tile ${image ? 'attachment-image' : ''}`}>
    {url ? <a className="attachment-download" href={url} download={attachment.name} title={t("{0} 다운로드", { 0: attachment.name })}>{contents}<Download size={13} aria-hidden="true" /></a> : <div className="attachment-local">{contents}</div>}
    {onRemove && <button className="attachment-remove" type="button" aria-label={t("{0} 첨부 제거", { 0: attachment.name })} disabled={disabled} onClick={() => onRemove(attachment.key)}><X size={13} aria-hidden="true" /></button>}
  </li>;
});
...
export function DraftAttachments({ attachments, onRemove, disabled }: { attachments: readonly DraftAttachment[]; onRemove: (key: string) => void; disabled: boolean }) {
  useI18n();
  return <ul className="attachment-list draft-attachments" aria-label={t("첨부할 파일")}>{attachments.map(attachment => <AttachmentTile key={attachment.key} attachment={attachment} onRemove={onRemove} disabled={disabled} />)}</ul>;
}

export const SavedAttachments = memo(function SavedAttachments({ attachments }: { attachments: readonly Attachment[] }) {
  useI18n();
  return <ul className="attachment-list saved-attachments" aria-label={t("보낸 첨부 파일")}>{attachments.map(attachment => <AttachmentTile key={attachment.id} attachment={savedAttachmentDraft(attachment)} />)}</ul>;
});
