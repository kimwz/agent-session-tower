...
export function WorkspacePage({ cwd, initialTool, toolRequest, onClose, registerGuard }: { cwd: string; initialTool: string; toolRequest?: number; onClose?: () => void; registerGuard?: (guard: () => boolean) => void }) {
  ...
  const openFile = async (path: string, reload = false) => {
    ...
    try { const result = await api<{ path: string; content: string; revision: string }>(`/api/workspace/file?${new URLSearchParams({ cwd, path })}`); if (request.current === revision) { setDocument({ ...result, saved: result.content }); setEditorVersion(value => value + 1); setFolder(path.split('/').slice(0, -1).join('/')); closeMobileExplorer(); } }
    catch (error) { if (request.current === revision) setError(message(error)); }
    finally { if (request.current === revision) setFileLoading(false); }
  };
  const post = (path: string, body: object) => api(path, { method: 'POST', headers: { 'Content-Type': 'application/json', [REQUEST_TOKEN_HEADER]: token }, body: JSON.stringify(body) });
  const save = useCallback(async () => {
    const current = documentRef.current;
    if (!current || !token || savingRef.current || fileLoading) return;
    savingRef.current = true; setSaving(true); setError('');
    try {
      const result = await api<{ revision: string }>('/api/workspace/file', { method: 'POST', headers: { 'Content-Type': 'application/json', [REQUEST_TOKEN_HEADER]: token }, body: JSON.stringify({ cwd, path: current.path, content: current.content, revision: current.revision }) });
  ...
  return <main className={`workspace-page${terminalMaximized ? ' terminal-maximized' : ''}`}>
    ...
    <div className="workspace-page-body"><aside id="workspace-explorer" hidden={!explorerVisible || terminalMaximized} className="workspace-explorer" aria-label={t('파일 탐색기')}><div className="workspace-explorer-toolbar"><strong>{t('파일')}</strong><button aria-label={t('새 파일')} title={t('새 파일')} disabled={!token || saving || fileLoading} onClick={() => { setCreation('file'); setNewName(''); }}><FilePlus size={16} /></button><button aria-label={t('새 폴더')} title={t('새 폴더')} disabled={!token || saving || fileLoading} onClick={() => { setCreation('directory'); setNewName(''); }}><FolderPlus size={16} /></button><button aria-label={t('새로고침')} title={t('새로고침')} onClick={() => setRefresh(value => value + 1)}><RefreshCw size={15} /></button></div>
      <div className="workspace-folder-path"><button disabled={saving || fileLoading} aria-pressed={!folder} aria-label={`${t('작업 폴더')}: ${cwd}`} title={cwd} onClick={() => { setFolder(''); setCreation(null); }}><Folder size={13} />/</button><span title={`${t('작업 폴더')}: ${folder || cwd}`}>{t('작업 폴더')}: {folder ? `/${folder}` : '/'}</span></div>
      ...
      {terminalStarted && token && <div className="workspace-terminal-pane" hidden={!terminalVisible}><WorkspaceTerminal cwd={cwd} token={token} maximized={terminalMaximized} onToggleMaximized={() => setTerminalMaximized(value => !value)} /></div>}</section></div>
  </main>;
}
