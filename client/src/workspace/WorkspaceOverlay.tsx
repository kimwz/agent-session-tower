...
export const WorkspaceContext = createContext<((cwd: string, tool: Tool) => void) | null>(null);
...
export function WorkspaceOverlayProvider({ children }: { children: ReactNode }) {
  const [workspace, setWorkspace] = useState<{ cwd: string; tool: Tool; request: number } | null>(null);
  ...
  const open = useCallback((cwd: string, tool: Tool) => {
    if (workspace && cwd !== workspace.cwd && !guard.current()) return;
    setWorkspace(previous => ({ cwd, tool, request: (previous?.request || 0) + 1 }));
  }, [workspace]);
  ...
  return <WorkspaceContext.Provider value={open}>{children}{workspace && <aside className={`workspace-overlay ${stacked ? 'is-stacked' : ''}`} aria-label={t('브라우저 작업 공간')} style={{ width, right }}>
    ...
    <Suspense fallback={<div className="chat-loading">{t('브라우저 작업 공간')}<button onClick={close}>{t('닫기')}</button></div>}><WorkspacePage key={workspace.cwd} cwd={workspace.cwd} initialTool={workspace.tool} toolRequest={workspace.request} onClose={close} registerGuard={registerGuard} /></Suspense>
