...
function Canvas({ slackUnreadIds, slack, selectedSlackId, onSelectSlack, triggerOverview, triggerEvents = noEvents, triggerUnreadIds, selectedTriggerEventId, onSelectTriggerEvent, triggerHasMore = false, onMoreTriggers, token = '', providers, hosts, hostsComplete = true, allHosts, sessions, allSessions = sessions, sessionsReady = true, unreadIds, selectedId, hostname, onSelect, onCanvasClick, filterKey, groups, visiblePins, groupSaving, groupErrors, groupActionsDisabled, onGroupUpdate, onGroupCreate, onAutoPrompt, repositories, onRepositoryAction, showHidden, onShowHiddenChange, settingsSuspended, emptyState }: GraphProps) {
  ...
  const { modelNodes, edges, shown } = useMemo(() => {
    ...
    const es: Edge[] = [];
    let x = 0;
    const monitorShown = showMonitor && machines.some(machine => !machine.node);
    const monitorItemCount = monitorShown ? monitorItems(slack?.connected ? slack.events : [], triggerEvents, slackLimit, selectedTriggerEventId ?? selectedSlackId).length : 0;
    const monitorWidth = monitorShown ? slackMentionLayout(monitorItemCount, monitorTotal - monitorItemCount > 0 || triggerHasMore).width : 0;
    let monitorAt = 0;
    ...
    machines.forEach((machine, machineIndex) => {
      ...
      ns.push({ id: hostId, type: 'host', position: hostPosition, data: { name: machine.name, active: sessions.filter(s => s.status === 'working' && s.node === machine.node).length, providers: machine.providers, disabled, onAutoPrompt: () => onAutoPrompt(undefined, machine.node ?? ''),
        ...(machine.node ? { link: { status: machine.status as HostLink['status'], live: machine.live, known: machine.known, canWork: machine.canWork, ...(machine.updating ? { updating: true } : {}), ...(machine.version ? { version: machine.version } : {}) } } : {}) }, style: { width: 256, height: HOST_HEIGHT, pointerEvents: 'all' }, zIndex: 20, draggable: manual, dragHandle: '.host-node', selectable: false, focusable: false });
    ...
    if (monitorShown) {
      ...
      ns.push({ id: TRIGGER_MONITOR_ID, type: 'triggerMonitor', position: slackPosition || { x: right, y: 185 }, style: { width: layout.width, height: layout.height }, zIndex: 1, draggable: true, dragHandle: '.slack-monitor-drag-handle', selectable: false, focusable: false,
        data: { ...(slack?.connected ? { slack: { name: slack.account?.teamName || 'Slack', enabled: slack.enabled, status: slack.status, error: slack.error } } : {}),
          enabledTriggers: triggerOverview?.triggers.filter(item => item.kind !== 'slack' && item.kind !== 'public' && item.enabled).length ?? 0, count: monitorTotal, active, remaining, more: triggerHasMore, onMore: showMoreSlack, onSelect: () => onSelectSlack?.(null) } });
