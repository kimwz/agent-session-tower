...
export function SlackPanel({ token, onClose, providers = [] }: { token: string; onClose: () => void; providers?: ProviderHealth[] }) {
  ...
  return createPortal(<dialog ref={dialog} className="slack-dialog" aria-labelledby="slack-title" onCancel={onClose}><div className="slack-panel"><header><div><h2 id="slack-title">{t('Slack 자동화')}</h2><p>{t('내 멘션을 읽고 작업한 뒤 답변을 제안합니다. Slack 전송은 채팅에서 사용자가 승인해야 합니다.')}</p></div><button className="icon-button" aria-label={t('닫기')} onClick={onClose}><X size={20} /></button></header>
...
export function SlackRules({ rules, onChange, providers = [] }: { rules: SlackRule[]; onChange: (rules: SlackRule[]) => void; providers?: ProviderHealth[] }) {
  ...
  return <>{rules.map((rule, index) => <article className="slack-rule" key={rule.id}><div className="slack-actions"><strong>{index + 1}</strong><label className="slack-check"><input type="checkbox" checked={rule.enabled} onChange={event => update(index, { enabled: event.target.checked })} />{t('활성')}</label><button type="button" className="icon-button" aria-label={t('지침 위로 이동')} disabled={!index} onClick={() => move(index, -1)}><ArrowUp size={16} /></button><button type="button" className="icon-button" aria-label={t('지침 아래로 이동')} disabled={index === rules.length - 1} onClick={() => move(index, 1)}><ArrowDown size={16} /></button><button type="button" className="icon-button" aria-label={t('지침 삭제')} onClick={() => onChange(rules.filter((_, i) => i !== index))}><Trash2 size={16} /></button></div><label>{t('지침 이름')}<input required value={rule.name} onChange={event => update(index, { name: event.target.value })} /></label><label>{t('일치 조건')}<textarea required rows={2} value={rule.condition} onChange={event => update(index, { condition: event.target.value })} /></label><label>{t('작업 지침')}<textarea required rows={3} value={rule.instructions} onChange={event => update(index, { instructions: event.target.value })} /></label><label>{t('답변 제안 가이드')}<small>{t('답변 후보를 작성할 때 참고합니다. Slack 전송에는 채팅에서 사용자의 승인이 필요합니다.')}</small><textarea required rows={2} value={rule.replyInstructions} onChange={event => update(index, { replyInstructions: event.target.value })} /></label><label className="slack-check"><input type="checkbox" checked={rule.autoReply === true} onChange={event => update(index, { autoReply: event.target.checked || undefined })} />{t('승인 없이 결과 자동 답변')}</label><small>{t('이 지침으로 위임한 작업이 끝나면 답변 가이드대로 결과를 한 번 전송하고, 요청 메시지에 진행 이모지를 달 수 있습니다. 채팅에서 “보내지 마세요”로 취소할 수 있습니다.')}</small><div className="slack-rule-options"><label>{t('에이전트')}<select value={rule.provider} onChange={event => update(index, { provider: event.target.value as SlackRule['provider'], model: undefined })}><option value="codex">Codex</option><option value="claude">Claude</option></select>{rule.provider === 'codex' && <small>{t('승인 검토: 항상 Auto')}</small>}</label><label>{t('모델')}<ModelPicker provider={providers.find(provider => provider.provider === rule.provider)} value={rule.model} onChange={model => update(index, { model })} /></label><label>{t('작업 폴더 (선택)')}<input value={rule.cwd || ''} onChange={event => update(index, { cwd: event.target.value || undefined })} /></label></div></article>)}</>;
}
