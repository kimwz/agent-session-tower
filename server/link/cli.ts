...
import { lockedPorts } from '../instance/state-lock.js';
...
export async function runLinkCommand(args: string[]): Promise<void> {
  ...
  console.log(`Connecting this computer to ${code.name} (fingerprint ${displayFingerprint(code.pin)}).`);
  let running = await runningTower(stateDir);
  if (!running) {
    if (!service) throw new Error('Tower is not running here. Start it (agent-session-tower --no-open) and run this command again, or leave out --no-service.');
    if (process.platform !== 'darwin') throw new Error('Tower is not running here. On this system, start it with your service manager (agent-session-tower --no-open) and run this command again.');
    ...
    console.log(`Tower ${code.version} now starts in the background whenever you log in. Waiting for it to start…`);
    ...
    console.log(`Tower ${running.version} is already running here (the other computer runs ${code.version}). Connecting with it.`);
  }
  ...
  const result = await waitFor(async () => {
    ...
    return item && (item.status === 'connected' && item.state === 'paired' || item.status === 'refused' || item.status === 'expired') ? item : undefined;
  }, 60_000);
  if (!result) throw new Error(`Still connecting to ${code.name}. Check that it can be reached at ${code.addresses.map(address => new URL(address).host).join(', ')}; this computer keeps trying.`);
  if (result.status !== 'connected') throw new Error(result.status === 'expired' ? 'The connection code expired before the computers met. Make a new one.' : `${result.error ?? 'Another computer answered at that address.'}`);
  console.log(`Connected. ${code.name} can now see and control this computer. Folders you exclude in Tower (Remote computers → Sharing) stay private.`);
...
async function runningTower(stateDir: string): Promise<{ base: string; version: string } | undefined> {
  for (const port of await lockedPorts(stateDir)) {
    const base = `http://127.0.0.1:${port}`;
    const health = await fetch(`${base}/api/health`, { signal: AbortSignal.timeout(1500) }).then(response => response.json() as Promise<{ ok?: boolean; version?: string }>).catch(() => undefined);
    if (health?.ok && health.version) return { base, version: health.version };
  }
  return undefined;
}
