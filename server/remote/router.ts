...
import { remoteJob, remoteJobVisible, remoteRun, remoteSession, remoteSessionIds, remoteSnapshot, type RemoteScope } from './visibility.js';
...
export function createRemoteRouter({ backend, exclusions, mutationsPerMinute = 60 }: RemoteRouterOptions) {
  ...
  const publish = () => {
    ...
    scheduled = setTimeout(() => {
      void prepare().catch(() => {}).then(() => { scheduled = undefined; for (const entry of streams.values()) entry.stream.publish(); });
    }, 200);
  };
  const unsubscribe = backend.subscribe(publish);
  exclusions.on('change', publish);
  ...
  const folderAllowed = async (cwd: string) => {
    if (!cwd.startsWith('/') || await exclusions.excludesNow(cwd)) throw httpError(404, '작업 폴더를 찾을 수 없습니다.');
  };
  ...
  const events = (req: Request, res: Response, principal: RemotePrincipal, patches: boolean) => {
    scope();
  ...
  const route = async (req: Request, res: Response, principal: RemotePrincipal, url: URL): Promise<void> => {
    ...
    if (method === 'GET' && detail) {
      ...
      return json(res, 200, { ...page, session: remoteSession(page.session) });
    }
    ...
    if (approval) {
      ...
      return json(res, 200, { run: remoteRun(await backend.respondToApproval(found.id, decodeURIComponent(approval[2]), response)) });
    }
    ...
    if (runAction) {
      ...
      if (runAction[2] === 'steer') {
        ...
        return json(res, 200, { run: remoteRun(await backend.steerRun(found.id)) });
      }
    ...
    if (path === '/api/auto-prompts') {
      ...
      if (request.cwd) await folderAllowed(request.cwd);
      ...
      return json(res, 202, { job: remoteJob(job, principal.controllerId, exclusions.revision) });
    }
  ...
  return {
    async handle(req: Request, response: ServerResponse | Http2ServerResponse, principal: RemotePrincipal): Promise<void> {
      ...
      try {
        ...
        const message = error instanceof Error && status < 500 ? error.message : status === 503 && error instanceof Error ? error.message : '요청을 처리하지 못했습니다.';
    ...
    dispose(): void {
      unsubscribe();
      exclusions.off('change', publish);
      exclusions.off('resolved', publish);
      clearInterval(heartbeat);
      if (scheduled) clearTimeout(scheduled);
      for (const id of [...streams.keys()]) this.disconnect(id);
    },
  };
}
