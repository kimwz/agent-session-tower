...
/** What this computer keeps between checks: the retries of a version that failed, and which failure was counted. */
interface Saved { retry?: RetryRecord; counted?: string }
...
export class TowerAutoUpdate {
  ...
  async start(): Promise<void> {
    ...
    this.saved = { ...(retry ? { retry } : {}), ...(typeof value?.counted === 'string' ? { counted: value.counted } : {}) };
  ...
  private async look(): Promise<void> {
    ...
    // Each failure of an update is counted once, when it is first seen here.
    if (status?.stage === 'failed' && status.startedAt !== this.saved.counted && newerVersion(status.version, version)) {
      await this.save({ retry: failedAgain(this.saved.retry, status.version, Date.parse(status.updatedAt) || this.now()), counted: status.startedAt });
    }
   ...
   * schedule says, and leaves the schedule as it is.
   */
  async updateNow(version?: string): Promise<UpdateRequestResult> {
    ...
    return this.options.updates.request(target);
  }
