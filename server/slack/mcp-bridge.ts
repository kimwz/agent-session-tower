...
export const SLACK_SESSION_TOOLS = [
  ...
  { name: 'tower_task_complete', description: 'Consume existing exact owner conditional reply authorization after verifying a delegated task outcome. Never creates permission or changes reply text. Completed process status alone is not success; provide actual evidence or mark failed/uncertain.', inputSchema: schema({ requestId: key, runId: key, outcome: { type: 'string', enum: ['succeeded', 'failed', 'uncertain'] }, evidence: { type: 'string', minLength: 1, maxLength: 4000 } }, ['requestId', 'runId', 'outcome', 'evidence']) },
