...
export function parseRunOrigin(value: unknown, references: 'required' | 'optional' = 'required'): RunOrigin | undefined {
  ...
  if (Object.keys(value).some(key => !['kind', 'workflowId', 'triggerId', 'eventId', 'runId'].includes(key))) return undefined;
...
export function restoredSessionOrigin(value: unknown): SessionOrigin | undefined {
  ...
  const origin = record(value) ? parseRunOrigin({ kind: value.kind, workflowId: value.workflowId, triggerId: value.triggerId, eventId: value.eventId }, 'optional') : undefined;
