...
import { socketOwner, tableEndpoint } from '../../../server/http/auth.js';
...
test('on Linux, a loopback connection is known by the owner of exactly that connection in the kernel’s socket table', () => {
  ...
  assert.equal(socketOwner(table, browser, server, 'LE'), 1000);
  ...
  assert.equal(socketOwner(waiting, browser, server, 'LE'), 1001);
  ...
  assert.equal(socketOwner(lookalike, browser, server, 'LE'), 1001);
  assert.equal(socketOwner([header, row('0100007F:C738', '0100007F:1F40', '01', 1000, 17), row('0100007F:C738', '0100007F:1F40', '01', 1001, 18)].join('\n'), browser, server, 'LE'), undefined,
    'two rows for one connection are not trusted');
  assert.equal(socketOwner(table, { address: '127.0.0.1', port: 0xC73A }, server, 'LE'), undefined, 'a connection it cannot find is not taken as local');
  assert.equal(socketOwner([header, row('0100007F:C738', '0100007F:1F40', '01', 1000, 21), row('0100007F:C738', '0100007F:1F40', '01', 1000, 21)].join('\n'), browser, server, 'LE'), 1000,
    'the same socket listed twice by a reading taken while sockets changed');
});

test('addresses are written as the kernel writes them, IPv6 and mapped IPv4 included', () => {
  assert.equal(tableEndpoint('127.0.0.1', 8000, 'LE'), '0100007F:1F40');
  assert.equal(tableEndpoint('127.0.0.1', 8000, 'BE'), '7F000001:1F40');
  assert.equal(tableEndpoint('::1', 8000, 'LE'), '00000000000000000000000001000000:1F40');
  assert.equal(tableEndpoint('::ffff:127.0.0.1', 8000, 'LE'), '0000000000000000FFFF00000100007F:1F40');
  const six = [header, row('0000000000000000FFFF00000100007F:C739', '0000000000000000FFFF00000100007F:1F40', '01', 33, 19)].join('\n');
  assert.equal(socketOwner(six, { address: '::ffff:127.0.0.1', port: 0xC739 }, { address: '::ffff:127.0.0.1', port: 8000 }, 'LE'), 33);
  // Read in the other byte order, a connection between two mirrored addresses would look like this server's own end.
  const mirrored = [header, row('7F01007F:1F40', '7F00017F:1F40', '08', 0, 20)].join('\n');
  assert.equal(socketOwner(mirrored, { address: '127.1.0.127', port: 8000 }, { address: '127.0.1.127', port: 8000 }, 'LE'), undefined, 'only this computer’s own byte order counts');
});
