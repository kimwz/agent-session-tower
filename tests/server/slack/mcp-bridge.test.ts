...
test('bound Slack MCP exposes only conversation tools and preserves request IDs', async () => {
  ...
  assert.deepEqual(replies[1].result.tools.map((tool: {name:string}) => tool.name), ['tower_auto_prompt','tower_task_status','tower_task_complete','slack_thread','slack_reply']);
