...
test('each joined computer says in one line where its update stands, and offers to try again after a failure', () => {
  ...
  assert.match(failed, /v1\.24\.0\(으\)로 계속 실행 중입니다\. 새 버전이 이 컴퓨터에 다시 연결하지 못했습니다\./);
  ...
  const cameBack = line(node({ report: { versions: { web: '1.24.0' }, service: true, update: update({ stage: 'failed', code: 'rollback-failed', failedStage: 'verifying' }) } }));
  assert.match(cameBack, /v1\.24\.0\(으\)로 계속 실행 중입니다/, 'running the previous version again, it came back');
  assert.match(cameBack, /다시 시도/);
...
test('a computer that cannot follow this Tower says why, and a newer one asks for this Tower to be updated', () => {
  assert.match(line(node({ features: ['read', 'work'] })), /한 번 직접 업데이트하세요/);
  assert.match(line(node({})), /지금 업데이트/, 'one that can follow, with no update under way, can be asked now');
